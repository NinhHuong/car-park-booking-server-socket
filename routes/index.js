...
io.sockets.on('connection', function (socket) {
    ...
    socket.on(constant.CONST.REQUEST_LOGIN_WITH_EMAIL_AND_PASS, function (email, password) {
        account.Login(email, password, function (res) {
            ...
            if (res.result) {
                accountID = res.id;
                dict[accountID] = idSave;
                console.log("account id " + accountID + " client id " + dict[accountID]);
            }
        });
    });
    ...
    //log out
    socket.on(constant.CONST.REQUEST_LOG_OUT, function (accountid) {
        account.LogOut(accountid, function (res) {
            socket.emit(constant.CONST.RESPONSE_LOG_OUT, res);

            if (res.result) {
                console.log("remote account " + accountid + " with ip " + dict[accountid]);
